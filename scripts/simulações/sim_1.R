...
# Gráficos para BSG
bsg_p1 <- prop_bsg |>
  dplyr::filter(type == "ewa" & sim_id == 1) |>
...
s1_bsg <- patchwork::guide_area() + (bsg_p1 + bsg_p2 + bsg_p3) +
...
s1_meg <- patchwork::guide_area() + (meg_p1 + meg_p2 + meg_p3) +
  patchwork::plot_layout(guides = "collect",
                         axis_titles = "collect",
                         nrow = 2,
                         heights = c(1, 10)) +
